    ...
    <header className="sticky top-0 z-50">
      ...
      <div
        className={`border-b transition-all duration-300 ${
          scrolled
            ? 'border-line bg-white/95 shadow-[0_4px_20px_rgba(20,35,59,0.10)] backdrop-blur-md backdrop-saturate-150'
            : 'border-transparent bg-white/90 backdrop-blur-md backdrop-saturate-150'
        }`}
      >
        <div className="flex items-center justify-between gap-5 px-[clamp(20px,6vw,80px)] py-3">
          <NavLink to="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
            <img
              src="/image/tag.png"
              alt="Ethiopian Emmanuel Evangelical Church logo"
              className="rounded-full object-cover border-2 border-gold shadow-[0_0_0_4px_rgba(201,162,75,0.12)]"
          ...
          <div className="hidden md:block">
            <NavLink to="/services" className="btn btn-gold !px-6 !py-2.5 text-sm">
        ...
        {open && (
          <nav className="md:hidden border-t border-line bg-white animate-rise">
            <ul className="flex flex-col py-2">
              ...
              <li className="px-[clamp(20px,6vw,40px)] pt-3 pb-2">
                <NavLink to="/services" onClick={() => setOpen(false)} className="btn btn-gold w-full">
                  <i className="fas fa-church" />
                  Join Us
                </NavLink>
              </li>
